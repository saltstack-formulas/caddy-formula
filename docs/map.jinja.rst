...
We will see a quick setup to configure the ``caddy`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``caddy`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``caddy`` formulas under ``/srv/salt/caddy/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/caddy/parameters/dns:domain/
    ln -s dns:domain /srv/salt/caddy/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/caddy/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/caddy/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``caddy`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``caddy`` formulas under ``/srv/salt/caddy/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/caddy/parameters/roles
...
We create a configuration for the role ``caddy/server`` in ``/srv/salt/caddy/parameters/roles/caddy/server.yaml``:
...
We create another configuration for the role ``caddy/client`` in ``/srv/salt/caddy/parameters/roles/caddy/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles caddy/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles caddy/client
...
.. code-block:: console
    ...
    mv /srv/salt/caddy/parameters/dns:domain/  '/srv/salt/caddy/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-caddy-as-json:
      test.nop:
        - name: {{ caddy | json }}
...
Here is an example based on `template-formula/caddy/config/file.sls`_:
...
This ``sls`` file expose a ``caddy`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ caddy['winner'] }}
...
.. _template-formula/caddy/config/file.sls: https://github.com/saltstack-formulas/template-formula/blob/master/caddy/config/file.sls
